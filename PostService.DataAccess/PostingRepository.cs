...
namespace PostService.DataAccess
{
    public class PostingRepository : IPostingRepository
    {
            ...
            while (reader.Read())
            {
                var posting = new Posting
                {
                    ...
                    CreatedAt = DateTimeOffset
                        .FromUnixTimeSeconds(reader.GetInt64(createdAtIndex))
                        .DateTime
                };
        ...
        public int Create(Posting posting)
        {
            ...
            conn.Open();
            const string sqlText = @"
                INSERT INTO Postings([From], [To], [Content],
                DeliveryType,
                Weight, Width, Height,
                Depth,
                [Value], [Price],
        ...
        public Posting? GetById(int postingId)
        {
            ...
            var posting = new Posting
            {
                ...
                DeliveryType = (DeliveryType)reader.GetInt32(
                    reader.GetOrdinal("DeliveryType")),
                ...
                Value = reader.IsDBNull(reader.GetOrdinal("Value"))
                    ? null
                    : reader.GetFloat(reader.GetOrdinal("Value")),
                Price = reader.GetFloat(reader.GetOrdinal("Price")),
                CreatedAt = DateTimeOffset
                    .FromUnixTimeSeconds(
                        reader.GetInt64(reader.GetOrdinal("CreatedAt")))
                    .DateTime
            };
        ...
        public int Update(Posting posting)
        {
            ...
            cmd.Parameters.AddWithValue(
                "@DeliveryType", (int)posting.DeliveryType);
            cmd.Parameters.AddWithValue("@Weight", posting.Weight);
            cmd.Parameters.AddWithValue("@Width", posting.Width);
            cmd.Parameters.AddWithValue("@Height", posting.Height);
            cmd.Parameters.AddWithValue("@Depth", posting.Depth);
            cmd.Parameters.AddWithValue(
                "@Value", posting.Value ?? (object)DBNull.Value);
            cmd.Parameters.AddWithValue("@Price", posting.Price);
